...
// ---- FalkorDB extension: schema DDL (index & constraint management) ----
oC_FalkorCommand
             :  oC_CreateIndex
                 | oC_DropIndex
                 | oC_CreateConstraint
...
oC_CreateConstraint
                :  CREATE SP CONSTRAINT SP ON SP? oC_IndexEntity SP ASSERT SP oC_Expression ( SP? ',' SP? oC_Expression )* ( SP oC_ConstraintPredicate )? ;

oC_DropConstraint
              :  DROP SP CONSTRAINT SP ON SP? oC_IndexEntity SP ASSERT SP oC_Expression ( SP? ',' SP? oC_Expression )* ( SP oC_ConstraintPredicate )? ;

oC_ConstraintPredicate
                   :  IS SP UNIQUE
                       | IS SP NOT SP NULL ;
...
// FalkorDB extension: CALL { ... } subquery (an inline nested query).
oC_CallSubquery
...
// FalkorDB extension: shortest-path pattern functions.
oC_ShortestPathPattern
                   :  ( SHORTESTPATH | ALLSHORTESTPATHS ) SP? '(' SP? oC_PatternElement SP? ')' ;
...
// FalkorDB extension: reduce(acc = init, x IN list | expr).
oC_ReduceExpression
...
oC_ExistentialSubquery
                   :  EXISTS SP? '{' SP? ( oC_RegularQuery | ( ( oC_ReadingClause SP? )+ ) | ( oC_Pattern ( SP? oC_Where )? ) ) SP? '}' ;
