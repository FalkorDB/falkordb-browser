...
export default function LoadUdf({ onSuccess }: LoadUdfProps) {
    ...
    const handleSubmit = async () => {
        ...
        try {
            const res = await securedFetch("/api/udf", {
                ...
                body: JSON.stringify({ name: name.trim(), code, replace }),
            }, toast, setIndicator);
        ...
        <Dialog open={open} onOpenChange={setOpen}>
            <DialogTrigger asChild>
                <Button
                    ...
                    className="bg-primary text-background p-2 rounded-lg w-full flex items-center justify-center gap-2"
            ...
            <DialogContent className="sm:max-w-md rounded-lg">
                <DialogHeader>
                    <DialogTitle>Load UDF</DialogTitle>
                    <DialogDescription>
                        Upload a JavaScript file as a User Defined Function.
                    </DialogDescription>
                </DialogHeader>
                <div className="flex flex-col gap-4">
                    <div className="flex flex-col gap-1">
                        <label className="text-sm" htmlFor="udf-name">Name</label>
                        <Input
                            id="udf-name"
                            value={name}
                    ...
                    <Button
                        label={isLoading ? "Loading..." : "Load UDF"}
