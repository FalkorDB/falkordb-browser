...
> {
  ...
  // ── Reconnect fallback ──
  // The Token DB entry for connId was stale or deleted (e.g. by a
  // concurrent signOut from another browser tab/test worker). Instead of
  // immediately returning SESSION_INVALID, try to re-establish a
  // connection using the session's credentials — exactly like the legacy
  // fallback above — so that transient Token DB gaps don't kick the user
  // out.
  try {
    const reconnectConnId = uuidv4();
    const reconnectKey = sessionConnectionKey(id, reconnectConnId);

    // Resolve password (best-effort, passwordless connections work without)
    ...
    try {
      ...
      const credentialRef = jwt?.credentialRef as string | undefined;
      if (credentialRef) {
        reconnectPassword = await getPasswordFromTokenDB(credentialRef);
      }
    } catch {
    ...
    // Persist so future requests go through the normal path
    try {
      const tokenHash = crypto
        .createHash("sha256")
        .update(`connection:${reconnectConnId}`)
        .digest("hex");
      await storeEncryptedCredential({
        tokenHash,
        tokenId: reconnectConnId,
        userId: id,
        username: session.user.username || "default",
        name: `connection:${reconnectConnId}`,
      ...
      // Non-fatal: connection works in-memory
    }
    ...
    // eslint-disable-next-line no-console
    console.warn("Reconnect fallback failed:", reconnectErr);
  }
